...
boolean ProcessUserInput3D(void) {
  ...
  if (g_bIKStatus == IKS_SUCCESS) {
    sIKX = min(max(g_sIKX + command.walkH/5, IK_MIN_X), IK_MAX_X);
    sIKY = min(max(g_sIKY + command.walkV/5, IK_MIN_Y), IK_MAX_Y);
    sIKZ = min(max(g_sIKZ + command.lookV/8, IK_MIN_Z), IK_MAX_Z);
